...
class GMM:
    ...
    def _update_distance_matrix(self, gram_vector, new_sample_norm, id1, id2, w1, w2):
        ...
        if id2 < 0:
            ...
            if alpha1 == 0:
                ...
                self._gram_matrix[id1, :] = self._gram_matrix[:, id1]
                self._gram_matrix[
                    id1, id1] = alpha1 ** 2 * norm_id1 + alpha2 ** 2 * new_sample_norm + 2 * alpha1 * alpha2 * \
                                gram_vector[id1]

            # update distance matrix
            self._distance_matrix[:, id1] = xp.maximum(
                self._gram_matrix[id1, id1] + xp.diag(self._gram_matrix) - 2 * self._gram_matrix[:, id1], 0)
            ...
            self._gram_matrix[id1, :] = self._gram_matrix[:, id1]
            self._gram_matrix[
                id1, id1] = alpha1 ** 2 * norm_id1 + alpha2 ** 2 * norm_id2 + 2 * alpha1 * alpha2 * ip_id1_id2
            ...
            self._distance_matrix[id1, :] = self._distance_matrix[:, id1]
            self._distance_matrix[id1, id1] = xp.inf
            self._distance_matrix[:, id2] = xp.maximum(
                self._gram_matrix[id2, id2] + xp.diag(self._gram_matrix) - 2 * self._gram_matrix[:, id2], 0)
    ...
    #     # update the training sample weights
    #     if frame_num == 1:
    #         replace_ind = 1
    ...
    def update_sample_space_model(self, samplesf, new_train_sample, num_training_samples):
        ...
        if num_training_samples == config.num_samples:
            ...
            if min_sample_weight < self.minimum_sample_weight:
                ...
                closest_existing_sample_pair = xp.unravel_index(closest_existing_sample_idx,
                                                                self._distance_matrix.shape)
                existing_samples_min_dist = self._distance_matrix[
                    closest_existing_sample_pair[0], closest_existing_sample_pair[1]]
                ...
                if closest_existing_sample1 == closest_existing_sample2:
                    raise Exception("Score matrix diagonal filled wrongly")
                ...
                if new_sample_min_dist < existing_samples_min_dist:
                    ...
                    self.prior_weights[closest_sample_to_new_sample] = self.prior_weights[
                                                                           closest_sample_to_new_sample] + config.learning_rate
                    ...
                    sample_to_merge2 = []
                    for i in range(num_feature_blocks):
                        sample_to_merge1.append(
                            samplesf[i][:, :, :, closest_existing_sample1:closest_existing_sample1 + 1])
                        sample_to_merge2.append(
                            samplesf[i][:, :, :, closest_existing_sample2:closest_existing_sample2 + 1])
        ...
        if abs(1 - xp.sum(self.prior_weights)) > 1e-5:
            raise Exception('weights not properly udpated')
