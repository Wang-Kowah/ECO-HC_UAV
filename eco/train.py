...
def inner_product_filter(xf, yf):
    ...
    for i in range(len(xf[0])):
        ip += 2 * xp.vdot(xf[0][i].flatten(), yf[0][i].flatten()) - xp.vdot(xf[0][i][:, -1, :].flatten(),
                                                                            yf[0][i][:, -1, :].flatten())
    return xp.real(ip)
...
def lhs_operation(hf, samplesf, reg_filter, sample_weights):
    ...
    for i in block_inds:
        ...
        sh[pad_sz[i][0]:output_sz[0] - pad_sz[i][0], pad_sz[i][1]:, :, :] += xp.matmul(hf[0][i].transpose(0, 1, 3, 2),
                                                                                       samplesf[i])
    ...
    for i in block_inds:
        hf_out[i] = xp.matmul(xp.conj(samplesf[i]),
                              sh[pad_sz[i][0]:output_sz[0] - pad_sz[i][0], pad_sz[i][1]:, :, :].transpose(0, 1, 3, 2))
...
def lhs_operation_joint(hf, samplesf, reg_filter, init_samplef, XH, init_hf, proj_reg):
    ...
    for i in block_inds:
        ...
        sh[pad_sz[i][0]:output_sz[0] - pad_sz[i][0], pad_sz[i][1]:, :, :] += xp.matmul(
            samplesf[i].transpose(0, 1, 3, 2), hf[i])
    ...
    for i in block_inds:
        # multiply with the transpose: A^H * BP
        hf_out[0][i] = hf_out1[i] + (
                    BP[pad_sz[i][0]:output_sz[0] - pad_sz[i][0], pad_sz[i][1]:, :, :] * xp.conj(samplesf[i]))

        # B^H * BP
        fBP[i] = (xp.conj(init_hf[i]) * BP[pad_sz[i][0]:output_sz[0] - pad_sz[i][0], pad_sz[i][1]:, :, :]).reshape(
            (-1, init_hf[i].shape[2]))

        # compute proj matrix part: B^H * A_m * f
        shBP[i] = (xp.conj(init_hf[i]) * sh[pad_sz[i][0]:output_sz[0] - pad_sz[i][0], pad_sz[i][1]:, :, :]).reshape(
            (-1, init_hf[i].shape[2]))
...
def train_filter(hf, samplesf, yf, reg_filter, sample_weights, sample_energy, reg_energy, CG_opts, CG_state):
    ...
    diag_M = [(1 - config.precond_reg_param) * (
                config.precond_data_param * m + (1 - config.precond_data_param) * xp.mean(m, 2, keepdims=True)) + \
...
def train_joint(hf, proj_matrix, xlf, yf, reg_filter, sample_energy, reg_energy, proj_energy, init_CG_opts):
    ...
    # construct preconditioner
    diag_M = [[], []]
    diag_M[0] = [(1 - config.precond_reg_param) * (
                config.precond_data_param * m + (1 - config.precond_data_param) * xp.mean(m, 2, keepdims=True)) + \
                 config.precond_reg_param * reg_energy_ for m, reg_energy_ in zip(sample_energy, reg_energy)]
    ...
    for iter_ in range(config.init_GN_iter):
        ...
        fyf = [xp.reshape(xp.conj(f) * yf_[:, :, xp.newaxis, xp.newaxis], (-1, f.shape[2])) for f, yf_ in
               zip(hf[0], yf)]  # matlab reshape
        ...
        hf, _, _ = preconditioned_conjugate_gradient(
            lambda x: lhs_operation_joint(x, init_samplef_proj, reg_filter, init_samplef, init_samplef_H, init_hf,
                                          config.projection_reg),  # A
            rhs_samplef,  # b
            init_CG_opts,  # opts
            lambda x: diag_precond(x, diag_M),  # M1
            None,  # M2
