...
class ECOTracker:
    ...
    def _get_reg_filter(self, sz, target_sz, reg_window_edge):
        ...
        if config.use_reg_window:
            ...
            reg_window = (reg_window_edge - config.reg_window_min) * (
                    np.abs(wrs / reg_scale[0]) ** config.reg_window_power +
            ...
            reg_window_dft[
                np.abs(reg_window_dft) < config.reg_sparsity_threshold * np.max(np.abs(reg_window_dft.flatten()))] = 0

            # do the inverse transform, correct window minimum
            reg_window_sparse = np.real(ifft2(reg_window_dft))
            reg_window_dft[0, 0] = reg_window_dft[0, 0] - np.prod(sz) * np.min(
                reg_window_sparse.flatten()) + config.reg_window_min
    ...
    def init(self, frame, bbox, total_frame=np.inf):
        ...
        if config.search_area_shape == 'proportional':
            ...
            self._img_sample_sz = np.ones((2), dtype=np.float32) * np.sqrt(
                np.prod(self._base_target_sz * config.search_area_scale))
        else:
        ...
        for idx, feature in enumerate(features):
            if feature['fname'] == 'cn' or feature['fname'] == 'ic':
                ...
                raise Exception("unimplemented features")
        ...
        sig_y = np.sqrt(np.prod(np.floor(self._base_target_sz))) * config.output_sigma_factor * (
                self._output_sz / self._img_sample_sz)
        yf_y = [np.sqrt(2 * np.pi) * sig_y[0] / self._output_sz[0] * np.exp(
            -2 * (np.pi * sig_y[0] * ky_ / self._output_sz[0]) ** 2)
                for ky_ in self._ky]
        yf_x = [np.sqrt(2 * np.pi) * sig_y[1] / self._output_sz[1] * np.exp(
            -2 * (np.pi * sig_y[1] * kx_ / self._output_sz[1]) ** 2)
                for kx_ in self._kx]
        ...
        if self._num_scales > 0:
            ...
            self._min_scale_factor = self._scale_step ** np.ceil(
                np.log(np.max(5 / self._img_sample_sz)) / np.log(self._scale_step))
            self._max_scale_factor = self._scale_step ** np.floor(
                np.log(np.min(frame.shape[:2] / self._base_target_sz)) / np.log(self._scale_step))
        ...
        xl = [x for feature in self._features
              for x in
              feature.get_features(frame, sample_pos, self._img_sample_sz, self._current_scale_factor)]  # get features
        ...
        for _ in range(config.refinement_iterations):
            ...
            if not np.allclose(old_pos, pos):
                ...
                xt = [x for feature in self._features
                      for x in
                      feature.get_features(frame, sample_pos, self._img_sample_sz, sample_scale)]  # get features
                ...
                xtf_proj = interpolate_dft(xtf_proj, self._interp1_fs,
                                           self._interp2_fs)  # interpolate features to continuous domain
